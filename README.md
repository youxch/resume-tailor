...
针对目标岗位 JD 自动分析匹配度、改写简历内容的 Claude Skill。

预览页：https://youxch.github.io/resume-tailor/

## 这是做什么的

换赛道找工作时最难的不是能力不够，而是 HR 和面试官看不出你的经历和这个岗位之间的关系——你需要把"你做过什么"翻译成"这个岗位想要什么"。

把目标岗位的 JD 粘进去，这个 skill 会：

1. **判断方向**：根据 JD 关键词判断这个岗位最匹配你的哪个专业方向（或两个方向的交叉点）
2. **三视角打分**：分别从 HR / 技术面试官 / 产品负责人 三个视角给当前简历内容打分，定位优势和差距
3. **自动改写并迭代**：重写项目描述、关键句，按打分结果反复修改，直到三个视角都达到目标分数再输出

## 怎么用

1. 把这个 skill 放到 `~/.claude/skills/`（或你的 Claude Code / Claude Desktop 的 skills 目录）
